...
- `IN PROGRESS（M3-A CM-3）`：结构化 Compaction Checkpoint 已作为 append-only Context Event 原子落盘；`prepare_turn` 会测量 canonical request footprint，并在已知输入预算达到 90% 或溢出时于 Provider 调用前停止；超过 16 KiB 的 Tool Result 会确定性替换为 512 字符预览与内容 hash 引用，完整内容以 SQLite content-addressed spill 与 Manifest 同事务提交，重启和 Project backup 后可校验恢复，完整 Transcript 始终保留；自动摘要/cut、通用 prune、有效压缩闭环和一次 overflow recovery 尚未完成；
...
- `NOT IMPLEMENTED（production）`：自动 summary/cut 与 overflow recovery、长期 Run 检索、Approval Grant/Run Budget、通用 Tool Registry/ToolExecution、Music Project Model、MIDI Tool、Sampler、Factory Pack、Audio Engine、VST3 Host 与 MCP Client。
