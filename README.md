...
- `NOT IMPLEMENTED（production）`：CM-4 长期 Run 检索、Approval Grant/Run Budget、通用 Tool Registry/ToolExecution、Music Project Model、MIDI Tool、Sampler、Factory Pack、Audio Engine、VST3 Host 与 MCP Client。超长 single-turn 在没有安全 cut 时会明确失败；Provider-specific 精确 tokenizer 和真实 overflow live qualification 仍待完成。
